...
**HC3 is available to play here:** https://lichess.org/@/HC2_18-20

When learning a new opening or practicing common middle games, it is useful to practice these positions over the board. However, games against human opponents rarely play into positions you may be interested in practicing. The reasonable alternative is to play the position with a computer. However, computers, even those designed to be at a specified skill level, play unrealistic, unnatural moves. HC3’s goal is to play the move that replicates **natural human behavior** the best, rather than best move in the position. For HC3, natural human behavior includes both move choice and move speed (how long it takes to play a move).

The most well-known chess engine is **Stockfish**, which uses search algorithms to find the move with the most favorable position at a certain depth. Neural network-based chess engines, such as **AlphaZero** and **Leela Chess Zero**, employ deep learning architectures and self-play based reinforcement learning strategies to learn from itself and optimize moves for favorable game outcomes. Recently, **Maia** employed a Leela-like CNN architecture to learn the game of chess. However, instead of optimizing for moves generated by self-play, it learned from real online Lichess games. Therefore, the moves Maia played were not necessarily the best move but the most natural move for the position. 

**HC3** builds on Maia with four key differences: 

1. **Transformer Architecture**: While Maia uses a CNN architecture originally from Leela, HC3 uses a ViT-like architecture adopted from newer generations of Leela.
2. **Secondary Model Inputs**: Aside from the raw board state, HC3 is directly provided with extra information that may otherwise be derivable from the raw board state. However, this avoids wasting valuable parameters on internally deriving this information from the raw board state. 
3. **Position Metadata**: General information of the game, such as each player’s ELO, time control, and increment, improve the model’s understanding beyond the board.
4. **Clock Integration**: The clock plays a crucial role in how humans play the game of chess. For each position, HC3 is given how much time it has and how much time the opponent has. HC3 also learns to predict how fast a move is played. This maintains the clock such that the time inputs are meaningful during gameplay. 

## Model Design
<img width="1354" alt="image" src="https://github.com/user-attachments/assets/feffcfec-188d-48d8-babe-35dae1e9b3a2" />

The model adopts a Vision Transformer architecture with 8 layers, 8 heads, 256 embedding dimensions, 256 hidden layer dimensions in the FFN, learned absolute positional encoding, and a GELU nonlinearity. Within each transformer block, Leela’s smolgen mechanism, described below, is used to generate relative positional encodings for each attention operation. I use 16 output dimensions for the first smolgen linear layer and 128 dimensions for the second smolgen linear layer.
...
### Position Encoding
Both absolute and relative position encodings are used. Relative position encoding comes from Leela's smolgen, which adjusts token importance based on board context (e.g., two bishops on the same diagonal in open positions). In computer vision applications, relative position encoding allows the model to decay how much two distant patches attend with one another. In chess, however, the necessity for long-distance attention is position-dependent. For example, a closed position with a locked center may not require much long-distance communication, while a rook or bishop endgame would. Therefore, chess requires relative position encodings to be aware of the position.

Very roughly, two linear layers compress the square encodings into `sg_hidden2` dimensions. Then, biases for each attention head is generated by two more linear layers. More details: [Leela Transformer Progress](https://lczero.org/blog/2024/02/transformer-progress/).
...
Some interesting observations from this. Accuracy improves as the game progresses, likely because there is less material so the moves become more obvious. This is evident in the material difference and material count plots. Move-matching accuracy remains relatively constant throughout each time control until 1-minute games. This is not an intuitive conclusion, because you would expect higher time pressure in blitz formats to make moves more random. Even more bizzare, move-matching accuracy remains constant even during time-pressure. However, a closer look needs to be taken, controlling for material count and other factors that may otherwise benefit move-matching accuracy.

Anecdotally, HC3 underperforms for its ELO class. Move timing is also somewhat strange and requires improvement, especially in slower formats. HC3 also has a tendency to randomly blunder, indicating that although it performs the classification task well, when it's wrong, it's wrong badly. For example, it often hangs a piece or mate in 1. For those interested, HC3 is available to play with [here](https://lichess.org/@/HC2_18-20). The ELO is set at 1900. Below is an example of HC3 doing this.
<div align="center">
...
- **Determinism**: Currently, the model chooses the most likely move, which is deterministic and can lead to dry games. Further testing with different temperatures during random sampling is required to optimize the playing experience. 
